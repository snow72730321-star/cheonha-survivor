...
ok(k.includes('player.critChance+=.0075')&&k.includes('+.0125')&&k.includes('+.0015')&&(k.includes('*=.995')||k.includes('*=.991')),"극월 24스택 성장치 누락");
...
for(const f of files){ok(fs.existsSync(path.join(root,f)),`왜도 VFX 누락: ${f}`);ok(loader.includes(f),`왜도 VFX 선택 무기 팩 누락: ${f}`)}
ok(!sw.includes('assets/vfx/skills/katana/full_moon.sheet.png')&&sw.includes('const APP_SHELL=['),"대용량 왜도 VFX가 작은 앱 셸에 다시 포함됨");
...
ok(k.includes('const ring=img("star_ring.png")')&&(k.includes('for(let i=0;i<n;i++)')||k.includes('for(let i=0;i<24;i++)'))&&k.includes('ctx.clip()'),"극월 star_ring 순차 점등 누락");
