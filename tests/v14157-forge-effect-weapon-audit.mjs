...
assert.match(forge,/const primary=window\.WeaponVisuals\?\.asset\?\.\(item\)\|\|`assets\/weapons\/master\/\$\{item\.weapon\}\.png`/);
assert.match(forge,/const fallback=window\.WeaponVisuals\?\.hudAsset\?\.\(item\)\|\|`assets\/weapons\/hud\/\$\{item\.weapon\}\.png`/);
assert.match(forge,/if\(effect\)scene\.classList\.add\(effect\);\s*renderAnvilSelection\(scene\)/);
assert.match(forge,/img\.onerror=\(\)=>\{[\s\S]*?img\.src=fallback/);
