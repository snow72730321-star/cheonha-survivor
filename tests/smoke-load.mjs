...
const katanaHitRegression=vm.runInContext(`(()=>{
  ...
  fireBasic();
  const oneShot={dead:one.dead,silverVfx:visuals.some(v=>v.type==="katanaSilverHit"),silverPopup:damagePopups.some(p=>p.silver)};
...
if(!katanaHitRegression.oneShot.dead||!katanaHitRegression.oneShot.silverVfx||!katanaHitRegression.oneShot.silverPopup||
