...
assert.match(forge,/return \{down:\.45,destroy:\.0075\}/,"고강화 하락·파괴 위험 누락");
assert.match(forge,/r<risk\.destroy\+risk\.down/,"강화 단계 하락 처리 누락");
assert.match(combat,/capMonsterOreGrade/);assert.match(combat,/gradeIndex\("legendary"\)/,"몹 광석 전설 상한 누락");
assert.match(storage,/cost:3000,mythic:\.002,eternal:\.00005/);assert.match(storage,/data-ore-gacha/);assert.ok(!storage.includes("account.ores[key]-=3"),"구형 전설 광석 소모형 뽑기가 남아 있음");
