...
for(const file of pngs){
  ...
  if(!relative.includes("/crests/")&&!relative.endsWith(".sheet.png")&&!renderer.includes(relative))throw new Error(`VFX 레지스트리 누락: ${relative}`);
  if(relative.endsWith(".sheet.png")&&!katanaRenderer.includes(path.basename(relative)))throw new Error(`왜도 GIF sheet 렌더러 매핑 누락: ${relative}`);
...
if(skillPngs.length!==40||hashes.size!==40)throw new Error(`무기군별 skills VFX가 누락되었거나 중복됨: ${skillPngs.length}개 / 고유 ${hashes.size}개`);
