...
for(const file of pngs){
  ...
  const katanaSheet=relative.includes("/katana/")&&relative.includes(".sheet");
  if(!relative.includes("/crests/")&&!katanaSheet&&!renderer.includes(relative))throw new Error(`VFX 레지스트리 누락: ${relative}`);
  if(katanaSheet&&!katanaRenderer.includes(path.basename(relative)))throw new Error(`왜도 GIF sheet 렌더러 매핑 누락: ${relative}`);
}
