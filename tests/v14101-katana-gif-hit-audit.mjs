...
for(const f of sheets){ok(fs.existsSync(path.join(root,"assets/vfx/skills/katana",f)),`GIF frame sheet 누락: ${f}`);ok(loader.includes(f),`왜도 선택 팩 누락: ${f}`)}
ok(sw.includes('fetchAndCache(request)')&&!sw.includes('assets/vfx/skills/katana/full_moon.sheet.png'),"VFX 런타임 캐시 정책 누락");
