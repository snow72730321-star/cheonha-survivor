...
async function cacheAppShell(){
  ...
  const results=await Promise.allSettled(APP_SHELL.map(path=>cache.add(new Request(path,{cache:"reload"}))));
  const failed=results.flatMap((result,index)=>result.status==="rejected"?[APP_SHELL[index]]:[]);
  if(failed.length)console.warn("앱 셸 일부 캐시 실패",failed);
  if(!await cache.match("index.html"))throw new Error("오프라인 시작 문서를 캐시하지 못했습니다.");
}
