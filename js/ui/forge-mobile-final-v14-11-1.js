...
(()=>{
  ...
  function renderWeaponInventoryFinal(){
    ...
    requestAnimationFrame(()=>{host.scrollTop=Math.min(previousScroll,Math.max(0,host.scrollHeight-host.clientHeight))});
  ...
  function applyWeaponFilter(){
    ...
    const bogu=weaponFilter==="bogu";window.BoguSystem?.setForgeMode?.(bogu);
    if(bogu)return;
  ...
  function syncWeaponInspect(){
    if(weaponFilter==="bogu"){window.BoguSystem?.renderForge?.();return}
  ...
  window.ForgeMobileUI={setScreen,renderOrePage,syncWeaponInspect,renderBrokenWeapons,updateGold,get screen(){return current},get weaponFilter(){return weaponFilter}};
