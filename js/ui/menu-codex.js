...
const CODEX_EVOLUTION_INFO={
 ...
 katana:{condition:"일섬 발도술 8성 + 월영연참 4성",desc:"치명타와 발도 속도를 강화하는 왜도 진화."},
...
function renderCodexView(){
 ...
 if(codexSelectedPath==="common"){
   ...
   <div class="codex-highlight-grid"><article class="codex-feature"><span>절기</span><strong>${ch?.ultimate||"전용 절기"}</strong><p>${ch?`${ch.name}의 전용 절기. 게이지 100%에서 발동한다.`:"전용 절기"}</p></article><article class="codex-feature evolution"><span>진화 무공</span><strong>${evo?.name||"미정"}</strong><p>${evoInfo?.desc||"조건을 완성하면 자동으로 진화한다."}</p><div class="codex-condition"><span>진화 조건</span><b>${evoInfo?.condition||"조건 정보 없음"}</b></div></article></div>
