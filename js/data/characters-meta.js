...
const evolutionDefs={
 sword:{name:"양의만상검",check:p=>(p.arts.qingfeng||0)>=8&&(p.arts.taiji||0)>=4,apply:p=>{p.projectileBonus+=1;p.areaMul*=1.08}},
 ...
 katana:{name:"월은신도",check:p=>(p.arts.iai||0)>=8&&(p.arts.moonchain||0)>=4,apply:p=>{p.critChance+=.08}},
...
const ultimateAssetPrimed=new Set();
function gainUltimate(v,{ignoreScaling=false}={}){
 ...
 if(player.ultimate>=60&&!ultimateAssetPrimed.has(selectedWeapon)){ultimateAssetPrimed.add(selectedWeapon);GameAssets.preloadUltimate?.(selectedWeapon).catch(error=>console.warn("절기 연출 준비 실패",error))}
 updateUltimateHud();
}
...
function setOneStrikeStacks(n){
 const old=player.oneStrikeStacks||0,next=Math.max(0,Math.min(100,n|0)),delta=next-old;
 if(delta<=0){player.oneStrikeStacks=next;return}
 player.oneStrikeStacks=next;
 // 일극은 해당 런이 끝날 때까지 영구 유지되는 성장치다.
 player.maxHp+=delta*.5;
 player.hp=Math.min(player.maxHp,player.hp+delta*.5);
...
const baseKillEnemy=killEnemy;killEnemy=function(e,source="basic",opt={}){
 ...
 if(!wasDead&&e.dead){
   ...
   if(selectedWeapon==="fist"&&source==="tenThousand"&&(player.arts?.diamondbody||0)>0)setOneStrikeStacks((player.oneStrikeStacks||0)+1);
   ...
   if(e.elitePrefix&&!e.type.includes("boss")){dropGold(e.x,e.y,4+Math.floor(5*player.luck));if(Math.random()<.10+player.luck*.05)dropOre(e.x+7,e.y,1)}
 }
};
