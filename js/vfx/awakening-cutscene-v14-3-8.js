...
(function installAwakeningCutscene(){
  ...
  // MP4는 GIF처럼 전체 프레임을 메모리에 펼치지 않아 iPhone의 순간 메모리 급증을 막는다.
  let ultimateMedia=cutscene.querySelector(".ultimate-cutin-gif");
  if(!ultimateMedia){
    ultimateMedia=document.createElement("video");
    ultimateMedia.className="ultimate-cutin-gif";
    ultimateMedia.muted=true;ultimateMedia.autoplay=true;ultimateMedia.playsInline=true;ultimateMedia.preload="metadata";
    ultimateMedia.setAttribute("playsinline","");ultimateMedia.setAttribute("webkit-playsinline","");ultimateMedia.setAttribute("disablepictureinpicture","");
    cutscene.prepend(ultimateMedia);
  }
  ...
  const gifByWeapon={
    ...
    poison:"assets/vfx/cutscenes/poison-chuhon-bijeop.gif",
    tao:"assets/vfx/cutscenes/tao-gucheon-noegeop.gif",
    saber:"assets/vfx/cutscenes/saber-cheonma-habil.gif",
    katana:"assets/vfx/cutscenes/katana-munen-issen.gif",
    fist:"assets/vfx/cutscenes/fist-hangryong-jincheon.gif"
  };
  const videoByWeapon=Object.fromEntries(Object.entries(gifByWeapon).map(([weapon,path])=>[weapon,path.replace(/\.gif$/,".mp4")]));
  ...
  useUltimate=function(){
    ...
    const gifSrc=gifByWeapon[selectedWeapon],videoSrc=videoByWeapon[selectedWeapon];
    cutscene.classList.toggle("ultimate-gif-active",!!videoSrc);
    if(videoSrc){
      const fallback=()=>{
        if(ultimateMedia.tagName!=="VIDEO")return;
        const image=document.createElement("img");image.className="ultimate-cutin-gif";image.alt="";image.decoding="async";image.draggable=false;image.src=gifSrc+"?assetBuild=v14.14.0-fallback";
        ultimateMedia.replaceWith(image);ultimateMedia=image;
      };
      if(ultimateMedia.tagName==="VIDEO"){
        ultimateMedia.pause?.();ultimateMedia.src=videoSrc+"?assetBuild=v14.14.0";ultimateMedia.currentTime=0;ultimateMedia.load?.();ultimateMedia.onerror=fallback;
        const playing=ultimateMedia.play?.();playing?.catch?.(fallback);
      }else ultimateMedia.src=gifSrc+"?assetBuild=v14.14.0-fallback";
    }
    ...
    setTimeout(()=>{
      ...
      if(ultimateMedia){ultimateMedia.pause?.();if(typeof ultimateMedia.removeAttribute==="function")ultimateMedia.removeAttribute("src");else ultimateMedia.src="";ultimateMedia.load?.()}
