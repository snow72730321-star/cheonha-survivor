...
globalThis.VFXSprites=(()=>{
  const definitions=Object.freeze({
    ...
    skillSaberUnityThunder:{src:"assets/vfx/skills/saber/unity_thunder.png",frameW:592,frameH:283,frames:7,cols:6,fps:16.67,blend:"lighter"},
    ...
    skillSaberUnityMountain:{src:"assets/vfx/skills/saber/unity_mountain.png",frameW:741,frameH:200,frames:6,cols:5,fps:11.11,blend:"lighter"},
    ...
    skillSpearOverlord:{src:"assets/vfx/skills/spear/overlord.png",frameW:386,frameH:277,frames:16,cols:8,fps:16.67,blend:"lighter"},
    ...
    skillBowRicochetSeal:{src:"assets/vfx/skills/bow/ricochet_seal.png",frameW:400,frameH:300,frames:37,cols:10,fps:20,blend:"lighter"},
    ...
    skillTaoFiveThunder:{src:"assets/vfx/skills/tao/five_thunder.png",frameW:420,frameH:288,frames:12,cols:6,fps:9.76,blend:"source-over"},
    ...
    skillFistGoldenDragon:{src:"assets/vfx/skills/fist/golden_dragon_fist.png",sheets:["assets/vfx/skills/fist/golden_dragon_fist.png","assets/vfx/skills/fist/golden_dragon_fist.part2.png","assets/vfx/skills/fist/golden_dragon_fist.part3.png"],framesPerSheet:6,frameW:1304,frameH:516,frames:17,cols:3,fps:15.74,blend:"source-over"},
    ...
    skillFistToOne:{src:"assets/vfx/skills/fist/fist_to_one.png",sheets:["assets/vfx/skills/fist/fist_to_one.png","assets/vfx/skills/fist/fist_to_one.part2.png","assets/vfx/skills/fist/fist_to_one.part3.png","assets/vfx/skills/fist/fist_to_one.part4.png"],framesPerSheet:6,frameW:1614,frameH:1040,frames:21,cols:2,fps:11.11,blend:"source-over"},
    skillFistGoldenCharge:{src:"assets/vfx/skills/fist/golden_dragon_charge.png",sheets:["assets/vfx/skills/fist/golden_dragon_charge.png","assets/vfx/skills/fist/golden_dragon_charge.part2.png"],framesPerSheet:13,frameW:822,frameH:663,frames:26,cols:4,fps:16.67,blend:"source-over"},
    skillFistGoldenBeam:{src:"assets/vfx/skills/fist/golden_dragon_beam.png",frameW:900,frameH:308,frames:16,cols:4,fps:18.18,blend:"source-over"},
  });

  const effects=[];
  const files=[...new Set(Object.values(definitions).flatMap(def=>def.sheets||[def.src]))];
  ...
  function imageFor(id,frame=0){
    const def=definitions[id];
    if(!def)return null;
    const sheetIndex=def.sheets?Math.min(def.sheets.length-1,Math.floor(frame/def.framesPerSheet)):0;
    const src=def.sheets?.[sheetIndex]||def.src,image=GameAssets.image(src);
  ...
  function draw(id,x,y,{age=elapsed,angle=0,scale=1,scaleX=1,scaleY=1,alpha=1,screen=false,frame=null,blend=null,flipX=false,flipY=false,loop=true,fpsOverride=null}={}){
    ...
    const image=imageFor(id,index);if(!image)return false;
    const sheetFrame=def.sheets?index%def.framesPerSheet:index;
    ...
    const cols=Math.max(1,def.cols||def.frames),sx=(sheetFrame%cols)*def.frameW,sy=Math.floor(sheetFrame/cols)*def.frameH;
  ...
  function spawn(id,x,y,options={}){
    ...
    const speed=Math.max(1,Number(account.settings?.gameSpeed||1)),quality=account.settings?.quality||"normal",raid=!!globalThis.SoloRaidMode?.active,mobile=innerWidth<=760;
    const qualityCap=quality==="low"?105:quality==="high"?180:145;
    let cap=Math.max(80,Math.floor(qualityCap*(speed>=3?.62:speed>=2?.76:speed>=1.5?.9:1)));if(raid&&mobile)cap=Math.min(cap,84);else if(raid)cap=Math.min(cap,118);
  ...
  return Object.freeze({definitions,files,load,preload,draw,drawOneShot,beam,spawn,update,drawEffects,clear,isRed,isPoison,isGold,imageFor});
})();
...
/* -------------------------------------------------------------------------- */
const impactVfxNext=new WeakMap();
const noisyImpactSources=new Set(["basic","taiji","tenk","poison","latentPoison","moonEclipseEcho","transcendThunderfall"]);
...
damageEnemy=function(enemy,damage,source,options={}){
  ...
  // 보스 다단히트는 동일한 hit flipbook을 초당 수십 개 만들지 않는다. 스킬 본체 VFX는 그대로 유지된다.
  let allowImpact=true;if(dealt>0&&(enemy.type==="boss"||enemy.type==="midboss"||enemy.raidBoss||enemy.raidPart)){const now=performance.now(),raid=!!globalThis.SoloRaidMode?.active,interval=noisyImpactSources.has(source)?(raid?100:75):(raid?72:52),next=impactVfxNext.get(enemy)||0;if(now<next)allowImpact=false;else impactVfxNext.set(enemy,now+interval)}
  // Dedicated skill sprites use skipVisual:true so their art remains visually exclusive.
  if(dealt>0&&allowImpact&&!options.skipVisual&&!options.skipImpactVfx){
