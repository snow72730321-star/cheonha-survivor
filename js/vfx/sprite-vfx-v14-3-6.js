...
drawVisuals=function(){
  ...
  const spearTypes=new Set(["cone","spearAura","windTunnel","dragonSpear","pressure"]);
  ...
  for(const v of visuals){
    ...
    if(v.type==="line")VFXSprites.beam(red?"beamRed":"beamBlue",v.x1,v.y1,v.x2,v.y2,Math.max(3,v.width||4),alpha);
    else if(v.type==="lightning"){
      VFXSprites.beam("beamBlue",v.x1,v.y1,v.x2,v.y2,Math.max(3,(v.width||2)*2.2),alpha);
      VFXSprites.draw("lightning",v.x2,v.y2,{age:(1-alpha)*.22,scale:.52+Math.min(.55,(v.width||2)/5),alpha});
      ...
      if(v.type==="ring"||v.type==="marker")VFXSprites.draw(red?"shockRed":"shockBlue",v.x,v.y,{age:(1-alpha)*.28,scale:Math.max(.25,(v.r||42)/80),alpha:alpha*.62});
    }else if(v.type==="heavyArc"){
      // 박도 벽력도법은 화살촉 실루엣이 없는 전용 중량 참격 스프라이트를 사용한다.
      VFXSprites.draw("saberHeavy",v.x,v.y,{age:(1-alpha)*.32,angle:v.a||0,scale:Math.max(.42,(v.r||72)/86),alpha});
    }else if(slashTypes.has(v.type)){
      VFXSprites.draw(red?"slashRed":"slashCyan",v.x,v.y,{age:(1-alpha)*.28,angle:v.a||0,scale:Math.max(.3,(v.r||45)/82),alpha});
      ...
      const n=v.count||2;
      for(let i=0;i<n;i++){
        const offset=(i-(n-1)/2)*18,angle=(v.a||0)+(i%2?Math.PI/2:0);
        const x=v.x+Math.cos(angle+Math.PI/2)*offset,y=v.y+Math.sin(angle+Math.PI/2)*offset;
        VFXSprites.draw(red?"slashRed":"slashCyan",x,y,{age:(1-alpha)*.25,angle,scale:Math.max(.45,(v.r||85)/100),alpha});
      }
      ...
      VFXSprites.draw("poisonCloud",v.x,v.y,{age:elapsed*.45,scale:Math.max(.28,(v.r||50)/78),alpha:alpha*.68,blend:"source-over"});
    }else if(v.type==="bowstring"){
      // 활시위의 곡선은 전용 화살 텍스처와 짧은 광선 리본으로 표현한다.
      const a=v.a||0,length=v.r||70;
      VFXSprites.beam("trailGold",v.x-Math.cos(a)*length*.35,v.y-Math.sin(a)*length*.35,v.x+Math.cos(a)*length*.35,v.y+Math.sin(a)*length*.35,5,alpha*.8);
      VFXSprites.draw("arrowGreen",v.x+Math.cos(a)*length*.28,v.y+Math.sin(a)*length*.28,{angle:a,scale:.62,alpha});
    }else if(v.type==="feather"){
      VFXSprites.draw("arrowGreen",v.x,v.y,{angle:v.a||0,scale:Math.max(.25,(v.r||20)/48),alpha});
      VFXSprites.draw("spark",v.x,v.y,{age:(1-alpha)*.2,scale:.3,alpha:alpha*.7});
    }else if(bladeTypes.has(v.type)){
      if(v.type==="fallingSword")VFXSprites.draw("blade",v.x,v.y-(alpha*80),{angle:Math.PI/2,scale:Math.max(.6,(v.width||5)/6),alpha});
      else drawBladeRing(v,alpha);
    }else if(v.type==="impact"||v.type==="crack"||v.type==="earthSplit"||v.type==="fanBurst"){
      VFXSprites.draw(red?"explosionBlood":gold?"explosionFire":"hitBlue",v.x,v.y,{age:(1-alpha)*.30,angle:v.a||0,scale:Math.max(.35,(v.r||35)/70),alpha});
    }else if(v.type==="focusLine"||v.type==="streak"){
      const length=v.r||100,a=v.a||0;
      VFXSprites.beam(red?"beamRed":"beamBlue",v.x,v.y,v.x+Math.cos(a)*length,v.y+Math.sin(a)*length,Math.max(3,v.width||3),alpha);
    }else if(v.type==="poisonMote")VFXSprites.draw("poisonCloud",v.x,v.y,{age:(1-alpha)*.4,scale:.18+Math.min(.25,(v.r||4)/14),alpha:alpha*.7,blend:"source-over"});
    else if(v.type==="ember"||v.type==="pressureDot")VFXSprites.draw("spark",v.x,v.y,{age:(1-alpha)*.22,scale:.22+Math.min(.4,(v.r||4)/12),alpha});
    else if(v.type==="afterimage")VFXSprites.draw(poison?"poisonCloud":"smoke",v.x,v.y,{age:(1-alpha)*.35,scale:Math.max(.18,(v.r||16)/48),alpha:alpha*.4,blend:"source-over"});
    else if(v.type==="text"){
      const s=ws(v.x,v.y);ctx.save();ctx.globalAlpha=alpha;ctx.fillStyle=v.color||"#fff";ctx.font="bold 13px system-ui";ctx.textAlign="center";ctx.fillText(v.text||"!",s.x,s.y-(1-alpha)*22);ctx.restore();
    }
  }
...
drawOrbit=function(){
  ...
  VFXSprites.draw("circleBlue",W/2,H/2,{screen:true,age:elapsed*.35,scale:radius/96,alpha:.28+.03*level});
  for(let i=0;i<count;i++){
