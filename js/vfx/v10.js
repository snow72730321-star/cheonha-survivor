...
function emitCastVfx(w,a,lv=1){const [c1,c2,c3]=vfxPalette(w),x=player.x,y=player.y,q=VFX10.quality();
 ...
 else if(w==="saber"){if(!((player.saberUnityTimer||0)>0))VFX10.push({type:"skillSaberThunderFan",x,y,a,r:88+lv*5,life:.3,max:.3,color:c1,width:12})}
...
function emitSkillVfx(id,lv=1){const [c1,c2,c3]=vfxPalette(),x=player.x,y=player.y,a=facingAngle();
 ...
 switch(id){
  ...
  case "mountain": if(!((player.saberUnityTimer||0)>0))VFX10.push({type:"skillSaberMountain",x,y,a,r:260+lv*20,life:.68,max:.68,color:c2,width:30+lv*3}); break;
