...
// v14.3.10: 기존 픽셀을 손대지 않고 프레임마다 2px 안전 여백을 둔 36×44 시트를 사용한다. 행동별 48×64 시트는 AnimationController가 우선 사용한다.
const spriteSheetMeta={frameW:36,frameH:44,contentW:32,contentH:40,padding:2,frames:4,dirs:{down:0,left:1,right:2,up:3}};
...
window.__CHEONHA_RENDERER_MODE__="external-png-assets";
function spriteFrame(img,dir,frame,cx,cy,scale=2.7,alpha=1,flashOn=false){if(!img||!img.complete||!img.naturalWidth)return false;const fw=spriteSheetMeta.frameW,fh=spriteSheetMeta.frameH,row=spriteSheetMeta.dirs[dir]??0,col=((frame%spriteSheetMeta.frames)+spriteSheetMeta.frames)%spriteSheetMeta.frames,z=(canvas&&ctx&&state!=="menu")?mobileCameraScale():1,sc=scale*z;ctx.save();ctx.imageSmoothingEnabled=false;ctx.globalAlpha=alpha;if(flashOn){ctx.filter="brightness(3) saturate(0)"}ctx.drawImage(img,col*fw,row*fh,fw,fh,Math.round(cx-fw*sc/2),Math.round(cy-(spriteSheetMeta.padding+spriteSheetMeta.contentH*.72)*sc),Math.round(fw*sc),Math.round(fh*sc));ctx.restore();return true}
...
function drawArticulatedPlayerFrame(img,pose,cx,cy,scale){
  ...
  const dx=Math.round(cx-fw*sc/2),dy=Math.round(cy-(spriteSheetMeta.padding+spriteSheetMeta.contentH*.72)*sc);
...
drawPlayer=function(){
  ...
  if(!drawAdvancedPlayerFrame(selectedWeapon,action,pose.dir,cx,cy,1)){
    // 행동별 고급 시트가 없으면 기존 외부 128×160 PNG 시트를 사용한다.
    // 외부 PNG까지 실패하더라도 drawPixelHero 구형 생성 로직으로는 돌아가지 않는다.
    if(!drawArticulatedPlayerFrame(img,pose,cx,cy,2.78))drawMissingAsset(cx,cy,selectedWeapon);
  }
