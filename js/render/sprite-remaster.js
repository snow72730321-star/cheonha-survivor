...
/**
 * 플레이어의 이동 상태를 시각적인 포즈 값으로 변환한다.
 * 실제 스프라이트 시트는 방향별 4프레임이며, 프레임 사이의 움직임은
 * 캔버스 변형으로 보간해 발 디딤과 상체 관성을 표현한다.
 *
 * 던전 액션 게임의 원리를 참고하되 특정 게임의 프레임을 복제하지 않는다.
 * 캐릭터 몸체는 회전시키지 않고, 4방향 스프라이트를 유지한다.
 */
function playerMotionPose(){
  const moving=player.moving&&Math.hypot(move.x,move.y)>.08;
  const speed=Math.min(1,Math.hypot(move.x,move.y));
  const cycle=elapsed*(moving?10.5+speed*3.5:2.2);
  const step=Math.sin(cycle*Math.PI*2);
  const impact=Math.max(0,Math.cos(cycle*Math.PI*2));
  const dir=facingDir();
  const horizontal=dir==="left"||dir==="right";
  const dodge=player.dodgeTimer>0;
  return {
    dir,moving,dodge,
    frame:moving?Math.floor(cycle)%4:0,
    bob:moving?-Math.abs(step)*1.8:Math.sin(elapsed*2.2)*.25,
    sway:moving?step*(horizontal?1.35:.55):0,
    lean:moving?(horizontal?Math.sign(move.x||1)*-.018:Math.sign(move.y||1)*.008):0,
    squash:dodge?.88:1-impact*.025,
    stretch:dodge?1.18:1+impact*.02
  };
}

/**
 * 캐릭터를 그린다.
 * - 머리 위 문자 화살표를 사용하지 않는다.
 * - 발밑의 작은 방향 쐐기로 방향만 보조한다.
 * - 회피 시 진행 방향 반대쪽에 잔상을 두어 속도감을 만든다.
 */
drawPlayer=function(){
  ...
  // 회피 잔상: 캐릭터 본체와 같은 방향 프레임을 낮은 투명도로 반복한다.
  if(pose.dodge&&img&&img.complete&&img.naturalWidth){
    const a=facingAngle();
    for(let i=3;i>=1;i--){
      ctx.save();
      ctx.globalAlpha=.08+i*.045;
      spriteFrame(img,pose.dir,pose.frame,cx-Math.cos(a)*i*10,cy-Math.sin(a)*i*10,2.78,ctx.globalAlpha,false);
      ctx.restore();
    }
  }
  ...
  ctx.translate(-cx,-cy);
  if(!spriteFrame(img,pose.dir,pose.frame,cx,cy,2.78,1,false)){
    ctx.restore();
    oldDrawPlayerV7();
    return;
  }
  ...
  // 방향 쐐기는 캐릭터 발밑의 진행 방향에 배치한다.
  const a=facingAngle(),z=mobileCameraScale();
  const markerX=cx+Math.cos(a)*22*z;
  const markerY=cy+16*z+Math.sin(a)*13*z;
  ctx.save();
  ctx.translate(markerX,markerY);
  ctx.rotate(a);
  ctx.fillStyle="rgba(205,241,255,.68)";
  ctx.beginPath();
  ctx.moveTo(6*z,0);
  ctx.lineTo(-4*z,-3.5*z);
  ctx.lineTo(-2*z,0);
  ctx.lineTo(-4*z,3.5*z);
  ctx.closePath();
  ctx.fill();
  ...
  if(player.shield>0){
    ...
    ctx.setLineDash([4,3]);
    ctx.beginPath();
    ctx.arc(cx,cy,31*z,0,Math.PI*2);
    ctx.stroke();
    ctx.restore();
  }
};
