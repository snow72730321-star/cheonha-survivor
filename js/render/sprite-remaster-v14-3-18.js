...
drawPlayer=function(){
  ...
  if(selectedWeapon==="fist"&&((player.arts?.dragonreturn||0)>0||(player.arts?.diamondbody||0)>0)){
    ...
    const h=player.huanglongStacks||0,m=player.tenThousandStacks||0,o=player.oneStrikeStacks||0;
...
buildWeaponMenu=function(){ui.weaponGrid.innerHTML="";Object.entries(weaponDefs).forEach(([id,w])=>{const ch=characterDefs[id],b=document.createElement("button");b.type="button";b.className="weapon-card asset-card";b.dataset.id=id;const cp=window.CombatPowerSystem?CombatPowerSystem.equippedPower(id):0;b.innerHTML=`<img src="${GameAssets.url(`assets/portraits/${id}.png`)}" alt="${ch.name} 도트 초상화"><div><b>${ch.name}</b><small>${ch.title} · ${w.name}<br>기본: ${w.basic.name}${cp?`<br><span class="weapon-cp">전투력 ${CombatPowerSystem.format(cp)}</span>`:""}</small></div>`;b.addEventListener("click",()=>{selectedWeapon=id;document.querySelectorAll(".weapon-card").forEach(x=>x.classList.toggle("selected",x.dataset.id===id));renderCharacterPreview();buildDifficultyMenu();updateStartButton()});ui.weaponGrid.appendChild(b)});renderCharacterPreview()};
const damagePopups=[];let hitStopV7=0,lastHitSoundV7=0;
const bossPopupMergeV7=new WeakMap(),hitStopNextV7=new WeakMap();
const noisyHitSourcesV7=new Set(["poison","latentPoison","taiji","tenk","basic","moonEclipseEcho","transcendThunderfall"]);
const heavyHitSourcesV7=new Set(["ultimate","overlord","demon","nameless","tenThousand","transcendSwordSea","transcendSpearBurst"]);
function popupLimitV7(){const mobile=innerWidth<=760,raid=!!globalThis.SoloRaidMode?.active;return raid?(mobile?20:28):(mobile?32:GameBalance.limits.maxDamagePopups)}
function addDamagePopupV7(e,dealt,dmg,source,opt,now){
 const crit=dealt>dmg*player.damageMul*1.28,silver=!!opt.silverMoon,bossLike=e.type==="boss"||e.type==="midboss"||e.raidBoss||e.raidPart;
 if(bossLike){let bySource=bossPopupMergeV7.get(e);if(!bySource){bySource=new Map();bossPopupMergeV7.set(e,bySource)}const key=String(source||"hit"),prev=bySource.get(key);if(prev&&prev.life>0&&now-(prev.lastMerge||0)<=140){prev.value+=dealt;prev.text=Math.round(prev.value);prev.life=Math.max(prev.life,.48);prev.max=Math.max(prev.max,.62);prev.crit=prev.crit||crit;prev.silver=prev.silver||silver;prev.lastMerge=now;return}if(damagePopups.length>=popupLimitV7())return;const item={x:e.x,y:e.y-8,vx:(Math.random()-.5)*12,vy:-34,life:.62,max:.62,value:dealt,text:Math.round(dealt),crit,silver,lastMerge:now};damagePopups.push(item);bySource.set(key,item);return}
 if(damagePopups.length<popupLimitV7())damagePopups.push({x:e.x,y:e.y-8,vx:(Math.random()-.5)*18,vy:-38,life:.62,max:.62,value:dealt,text:Math.round(dealt),crit,silver,lastMerge:now});
}
function requestHitStopV7(e,dealt,source,opt,now){
 if(opt.skipHitStop||dealt<=0)return;const bossLike=e.type==="boss"||e.type==="midboss"||e.raidBoss||e.raidPart,heavy=heavyHitSourcesV7.has(source)||dealt>=(bossLike?150:95);if(!heavy||noisyHitSourcesV7.has(source)&&dealt<(bossLike?220:140))return;
 const next=hitStopNextV7.get(e)||0,cooldown=bossLike?125:90;if(now<next)return;hitStopNextV7.set(e,now+cooldown);hitStopV7=Math.max(hitStopV7,bossLike?.012:.009);
}
const baseDamageEnemyV7=damageEnemy;damageEnemy=function(e,dmg,source,opt={}){const before=e.hp;baseDamageEnemyV7(e,dmg,source,opt);const dealt=Math.max(0,before-Math.max(0,e.hp)),now=performance.now();if(dealt>0&&account.settings?.damageNumbers!==false)addDamagePopupV7(e,dealt,dmg,source,opt,now);requestHitStopV7(e,dealt,source,opt,now);const heavyHit=e.type==="boss"||e.type==="midboss"||e.raidBoss||dealt>55;if(heavyHit&&now-lastHitSoundV7>135){lastHitSoundV7=now;GameAudio.playSFX(e.type==="boss"||e.type==="midboss"||e.raidBoss?"boss-hit":"enemy-hit")}return dealt};
const baseUpdateV7=update;update=function(dt){if(hitStopV7>0){hitStopV7-=dt;dt*=.35}baseUpdateV7(dt);for(const p of damagePopups){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=35*dt;p.life-=dt}for(let i=damagePopups.length-1;i>=0;i--)if(damagePopups[i].life<=0)damagePopups.splice(i,1)};
function drawDamagePopupsV7(){const mobile=innerWidth<=760,raid=!!globalThis.SoloRaidMode?.active;for(const p of damagePopups){const s=ws(p.x,p.y),a=Math.max(0,p.life/p.max);ctx.save();ctx.globalAlpha=a;ctx.textAlign="center";const silver=!!p.silver,size=(silver?18:p.crit?17:13)*(account.settings?.damageNumberSize||1);ctx.font=`${silver||p.crit?"bold ":""}${size}px ui-monospace,monospace`;ctx.lineWidth=silver?4:3;ctx.strokeStyle=silver?"rgba(17,25,34,.94)":"#11150f";if(silver&&!raid){ctx.shadowColor="#dff8ff";ctx.shadowBlur=mobile?5:9}ctx.strokeText(p.text,s.x,s.y);ctx.fillStyle=silver?"#f8fdff":p.crit?"#ffe071":"#f4eee0";ctx.fillText(p.text,s.x,s.y);ctx.restore()}};
