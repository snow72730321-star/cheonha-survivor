"use strict";
/** v14.3.13 플레이어 프레임 중심축 및 하단 실루엣 복원 */
/* ===== v7 external sprite assets, hit feedback, boss warning ===== */
// v14.3.13: 적은 기존 32×40, 플레이어는 중심 보정과 발끝 복원을 포함한 36×48 시트를 사용한다.
const spriteSheetMeta={frameW:32,frameH:40,frames:4,dirs:{down:0,left:1,right:2,up:3}};
const playerSpriteSheetMeta={frameW:36,frameH:48,contentH:44,padTop:1,padBottom:3,frames:4,dirs:{down:0,left:1,right:2,up:3}};
...
function drawArticulatedPlayerFrame(img,pose,cx,cy,scale){
  ...
  const fw=playerSpriteSheetMeta.frameW,fh=playerSpriteSheetMeta.frameH;
  const row=playerSpriteSheetMeta.dirs[pose.dir]??0;
  const col=((pose.frame%playerSpriteSheetMeta.frames)+playerSpriteSheetMeta.frames)%playerSpriteSheetMeta.frames;
  ...
  const opaqueBottom=fh-playerSpriteSheetMeta.padBottom;
  ctx.drawImage(
...
drawPlayer=function(){
  ...
  if(!drawAdvancedPlayerFrame(selectedWeapon,action,pose.dir,cx,cy,1)){
    // 행동별 고급 시트가 없으면 중심·발끝 복원된 외부 144×192 PNG 시트를 사용한다.
