...
const WeaponVisuals=(()=>{
  ...
  function renderAnvilWeapon(node,item){
    ...
    let aura=node.querySelector(":scope > .anvil-weapon-aura");
    if(!aura){aura=document.createElement("div");aura.className="anvil-weapon-aura";node.prepend(aura)}
    let media=node.querySelector(":scope > .anvil-weapon-media");
    if(!media){
      media=document.createElement("div");media.className="anvil-weapon-media";
      const existing=node.querySelector(":scope > img.anvil-weapon-img");
      if(existing)media.append(existing);
      node.insertBefore(media,node.querySelector(":scope > .anvil-weapon-meta"));
    }
    let img=media.querySelector("img.anvil-weapon-img");
...
// 강화/속성 오오라는 무기 이미지 자체에만 유지한다. 플레이어 몸 주변에는 별도 오오라를 그리지 않는다.
