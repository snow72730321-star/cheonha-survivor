 ...
 */
const GameAssets=(()=>{
  const BUILD="v14.15.1-combat-safety-balance-ui";
  ...
  const sharedVfxFiles=vfxFiles.filter(path=>!path.includes("/skills/")&&!path.includes("assets/ui/katana/"));
  ...
  const weaponPacks=Object.freeze(Object.fromEntries(characterIds.map(id=>[id,Object.freeze(vfxFiles.filter(path=>path.includes(`/skills/${id}/`)||(id==="katana"&&path.includes("assets/ui/katana/"))))])));
