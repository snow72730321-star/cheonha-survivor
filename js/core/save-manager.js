...
const SaveManager=(()=>{
  ...
  function sanitizePotentials(value){
    ...
    const allowed={damage:"pct",crit:"pct",critDamage:"pct",boss:"pct",speed:"pct",reduction:"pct",attackSpeed:"pct",cooldown:"pct",area:"pct",hp:"flat",projectile:"flat",pierce:"flat"};
    return value.slice(0,3).map(line=>{
      const key=safeId(line?.key,"damage"),format=allowed[key]||"pct";
      const max=format==="pct"?1:key==="hp"?500:10;
      return {key,name:safeString(line?.name,"잠재",40),value:finite(line?.value,0,0,max),format,grade:grades.includes(line?.grade)?line.grade:"rare"};
    });
  }
  ...
  function sanitizeWeapon(item,index){
    ...
    return {
      ...
      level:Math.floor(finite(item?.level,0,0,25)),
      failStack:Math.floor(finite(item?.failStack,0,0,5)),
      artisanBreath:finite(item?.artisanBreath,Math.floor(finite(item?.failStack,0,0,5))*10,0,100),
  ...
  function sanitizeSettings(value){
    ...
    const gameSpeed=[1,1.5,2,3].includes(Number(input.gameSpeed))?Number(input.gameSpeed):1;
    return {
      quality,
      fps,
      shake:finite(input.shake,1,0,1),
      damageNumbers:input.damageNumbers!==false,
      masterVolume:finite(input.masterVolume,.8,0,1),
      bgmVolume:finite(input.bgmVolume,.55,0,1),
      sfxVolume:finite(input.sfxVolume,.8,0,1),
      uiVolume:finite(input.uiVolume,.85,0,1),
  ...
  const BOGU_POSITIVE_IDS=new Set(["damage","crit","boss","cooldown","reduction","hp","area","speed"]);
  const BOGU_NEGATIVE_IDS=new Set(["damageDown","critDown","cooldownDown","speedDown"]);
  const BOGU_GRADES=new Set(["common","rare","epic","unique","legendary","mythic","eternal"]);
  function sanitizeBoguGem(item,index){
    const oreType=Object.prototype.hasOwnProperty.call(oreTypes,item?.oreType)?item.oreType:"han";
    const grade=BOGU_GRADES.has(item?.grade)?item.grade:"common";
    const positive=Array.isArray(item?.positive)?item.positive.slice(0,2):[];
    while(positive.length<2)positive.push({id:"damage",successes:0});
    return {
      id:safeId(item?.id,`bogu-${Date.now()}-${index}`),oreType,grade,
      positive:positive.map((line,i)=>({id:BOGU_POSITIVE_IDS.has(line?.id)?line.id:(i?"crit":"damage"),successes:Math.floor(finite(line?.successes,0,0,10))})),
      negative:{id:BOGU_NEGATIVE_IDS.has(item?.negative?.id)?item.negative.id:"damageDown",successes:Math.floor(finite(item?.negative?.successes,0,0,10))},
      created:finite(item?.created,Date.now(),0,Number.MAX_SAFE_INTEGER)
    };
  }
  function sanitizeBoguFaceting(value){
    if(!value||typeof value!=="object")return null;
    const oreType=Object.prototype.hasOwnProperty.call(oreTypes,value.oreType)?value.oreType:"han",grade=BOGU_GRADES.has(value.grade)?value.grade:"common";
    const pos=Array.isArray(value.positive)?value.positive.slice(0,2):[];while(pos.length<2)pos.push(pos.length?"crit":"damage");
    const positive=pos.map((id,i)=>BOGU_POSITIVE_IDS.has(id)?id:(i?"crit":"damage"));
    if(positive[0]===positive[1])positive[1]=positive[0]==="crit"?"damage":"crit";
    const negative=BOGU_NEGATIVE_IDS.has(value.negative)?value.negative:"damageDown";
    const attempts=Array.isArray(value.attempts)?value.attempts.slice(0,3):[],successes=Array.isArray(value.successes)?value.successes.slice(0,3):[],results=Array.isArray(value.results)?value.results.slice(0,3):[];
    while(attempts.length<3)attempts.push(0);while(successes.length<3)successes.push(0);while(results.length<3)results.push([]);
    const cleanResults=results.map((row,i)=>Array.isArray(row)?row.slice(0,10).map(x=>x?1:0).slice(0,Math.floor(finite(attempts[i],0,0,10))):[]);
    return {id:safeId(value.id,`bf-${Date.now()}`),oreType,grade,positive,negative,chance:finite(value.chance,.75,.25,.75),attempts:attempts.map(x=>Math.floor(finite(x,0,0,10))),successes:successes.map((x,i)=>Math.floor(finite(x,0,0,Math.floor(finite(attempts[i],0,0,10))))),results:cleanResults,created:finite(value.created,Date.now(),0,Number.MAX_SAFE_INTEGER)};
  }
  ...
  function sanitizeAccount(raw){
    ...
    const uniqueId=(item,index,prefix)=>{
      const proposed=safeId(item?.id,"");
      if(proposed&&!usedIds.has(proposed)){usedIds.add(proposed);return proposed}
      let recovered=`recovered-${prefix}-${Date.now()}-${index}`;
      while(usedIds.has(recovered))recovered+=`-${usedIds.size}`;
      usedIds.add(recovered);return recovered;
    };
    const weapons=(Array.isArray(source.weapons)?source.weapons:[]).slice(0,500).map((item,index)=>Object.assign(sanitizeWeapon(item,index),{id:uniqueId(item,index,"weapon")}));
    const brokenWeapons=(Array.isArray(source.brokenWeapons)?source.brokenWeapons:[]).slice(0,500).map((item,index)=>{
      const weapon=sanitizeWeapon(item,index+1000);
      weapon.id=uniqueId(item,index,"broken");
      weapon.brokenAt=finite(item?.brokenAt,Date.now(),0,Number.MAX_SAFE_INTEGER);
      return weapon;
    });
    ...
    const weaponNames=new Set(Object.keys(weaponDefs));
    const validSkins=typeof skinDefs!=="undefined"?new Set(Object.keys(skinDefs)):new Set(["default"]);
    const validAchievements=typeof achievementDefs!=="undefined"?new Set(achievementDefs.map(item=>item.id)):new Set();
    const skinsUnlocked={default:true};
    for(const [id,unlocked] of Object.entries(plainObject(source.skinsUnlocked)))if(validSkins.has(id)&&unlocked)skinsUnlocked[id]=true;
    const selectedSkins={};
    for(const [weapon,id] of Object.entries(plainObject(source.selectedSkins)))if(weaponNames.has(weapon)&&validSkins.has(id))selectedSkins[weapon]=id;
    const achievements={};
    for(const [id,done] of Object.entries(plainObject(source.achievements)))if(validAchievements.has(id)&&done)achievements[id]=true;
    const paths={};
    for(const [weapon,played] of Object.entries(plainObject(stats.paths)))if(weaponNames.has(weapon)&&played)paths[weapon]=true;
    const pendingSource=plainObject(source.pendingPotential);
    const pendingLines=sanitizePotentials(pendingSource.lines);
    const pendingPotential=weaponIds.has(pendingSource.weaponId)&&["rare","epic","unique","legendary"].includes(pendingSource.grade)&&pendingLines.length===3?{
      weaponId:pendingSource.weaponId,grade:pendingSource.grade,lines:pendingLines,createdAt:finite(pendingSource.createdAt,Date.now(),0,Number.MAX_SAFE_INTEGER)
    }:null;
    const boguGems=(Array.isArray(source.boguGems)?source.boguGems:[]).slice(0,200).map(sanitizeBoguGem);
    const boguIds=new Set(boguGems.map(g=>g.id)),boguEquippedRaw=Array.isArray(source.boguEquipped)?source.boguEquipped.slice(0,3):[];
    while(boguEquippedRaw.length<3)boguEquippedRaw.push(null);
    const boguEquipped=boguEquippedRaw.map(id=>boguIds.has(id)?id:null);
    return {
      saveVersion:VERSION,
      ...
      boguGems,
      boguEquipped,
      boguFaceting:sanitizeBoguFaceting(source.boguFaceting),
      brokenWeapons,
      pendingPotential,
  ...
  async function importFile(file){
    ...
    if(Number(file.size)>MAX_IMPORT_BYTES)throw new Error("세이브 파일은 5MB 이하만 가져올 수 있습니다.");
