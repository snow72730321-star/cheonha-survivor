...
damageEnemy=function(entity,amount,source="unknown",options={}){
  ...
  if(dealt>0){
    ...
    if(GameEvents.has?.("enemy:damaged"))GameEvents.emit("enemy:damaged",{entity,dealt,source,options});
  }
  return result;
};
...
killEnemy=function(entity,source="basic",options={}){
  ...
  if(alive&&entity.dead){
    ...
    if(GameEvents.has?.("enemy:killed"))GameEvents.emit("enemy:killed",{entity,source,options});
  }
  return result;
};
...
loop=function fixedTimestepLoop(now){
  ...
  const activeRender=state==="playing"||state==="cutscene"||state==="levelup"||state==="augment";
  const targetFps=activeRender?Number(account.settings?.fps||60):state==="paused"?12:8;
...
function clearRunTransientObjects(){
  for(const item of projectiles)GamePools.projectile.release(item);
  for(const item of particles)GamePools.particle.release(item);
  enemies.length=projectiles.length=gems.length=particles.length=visuals.length=fields.length=delayed.length=hazards.length=chests.length=0;
  boss=null;bossSpawned=false;VFXSprites.clear?.();GameSpatial.rebuild([]);
}
document.getElementById("quitBtn")?.addEventListener("click",clearRunTransientObjects);
document.getElementById("menuBtn")?.addEventListener("click",clearRunTransientObjects);

function persistActiveRun(){
  if(["playing","paused","cutscene","levelup","augment"].includes(state))saveAccountData();
}
window.setInterval?.(persistActiveRun,30000);
window.addEventListener("pagehide",persistActiveRun,{capture:true});
document.addEventListener("visibilitychange",()=>{if(document.hidden)persistActiveRun()});
