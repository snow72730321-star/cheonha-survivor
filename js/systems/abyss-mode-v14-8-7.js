...
const AbyssModeV1487=(()=>{
  ...
  const baseAbyssKillEnemy=killEnemy;
  killEnemy=function(entity,source="basic",options={}){
    const eligible=active()&&entity?.type==="midboss"&&!entity.dead;
    const result=baseAbyssKillEnemy.apply(this,arguments);
    if(eligible&&entity.dead&&Math.random()<.10){dropWeaponSoulStone(entity.x+12,entity.y-12);showMessage("무한나락 중간보스 · 무혼석 드랍",1.4)}
    return result;
  };
