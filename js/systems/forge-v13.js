...
(function installForgeV13(){
  ...
  // 강화 실패 후 결과 확률. 파괴는 낮게 유지하되 +10 이후 단계 하락은 무시하기 어렵게 설계한다.
  function failureRisk(item){
    const lv=Math.max(0,Number(item?.level)||0);
    if(lv<5)return {down:0,destroy:0};
    if(lv<10)return {down:.10,destroy:0};
    if(lv<15)return {down:.20,destroy:0};
    if(lv<20)return {down:.32,destroy:.0025};
    return {down:.45,destroy:.0075};
  }
  ...
  function openDetail(id,tab="enhance"){
    ...
    normalizeWeapon(item); const root=detailOverlay();root.dataset.forgeMode=tab;root.classList.add("show"); switchTab(tab); renderDetail();
  }
  ...
  function renderDetail(){
    ...
    const risk=failureRisk(item),riskBox=root.querySelector("#enhanceRisk");if(riskBox)riskBox.textContent=item.level>=MAX_ENHANCE?"최대 강화 단계":`실패 시 단계 하락 ${(risk.down*100).toFixed(1)}% · 파괴 ${(risk.destroy*100).toFixed(2)}% (실패 결과 기준)`;
  ...
  function executeEnhance(){
    ...
    account.gold-=cost; updateForgeLiveGold(); scene.className="anvil-scene striking"; message.className="forge-result-message"; message.textContent="장인이 호흡을 가다듬고 망치를 내리친다…"; GameAudio.playUI("forge-strike");
    setTimeout(()=>{
      ...
      if(success){
        ...
        const risk=failureRisk(item),r=Math.random();item.failStack=Math.min(5,item.failStack+1);
        scene.className="anvil-scene failure";message.className="forge-result-message bad";
        if(r<risk.destroy){
          const name=item.name,id=item.id,idx=account.weapons.findIndex(x=>x.id===id);if(account.equipped[item.weapon]===id)delete account.equipped[item.weapon];if(idx>=0)account.weapons.splice(idx,1);selectedItemId=null;
          message.textContent=`강화 실패 · ${name}이 파괴되었다.`;GameAudio.playUI("forge-failure");saveAccountData();renderWeaponInventory();setTimeout(()=>{detailOverlay().classList.remove("show");refreshForge()},700);return;
        }else if(r<risk.destroy+risk.down&&item.level>0){
          item.level--;item.damageMul=item.baseDamageMul*enhancementMultiplier(item.level,item);message.textContent=`강화 실패 · 단계 하락. ${item.name}이 +${item.level}이 되었다.`;
        }else{
          const pityStep=item.level>=20?.6:item.level>=15?1.2:5;message.textContent=`강화 실패. 장인의 숨결이 ${(item.failStack*pityStep).toFixed(item.level>=15?1:0)}%p 누적됐다.`;
        }
  ...
  window.renderWeaponInventory=function(){
    ...
    ui.weaponInventory.innerHTML=account.weapons.length?account.weapons.slice().reverse().map(item=>{
      ...
      return `<div class="weapon-item v13-item art-weapon-card ${eq?"equipped":""}" data-weapon="${item.weapon}" data-select-weapon="${item.id}" style="--weapon-aura:${color}"><div class="weapon-card-art" data-art-id="${item.id}"><img src="${art}" alt="${item.name}"></div><div class="weapon-card-copy"><strong><span class="rarity-${item.grade}">${gd.name} ${item.name} +${item.level}</span><span>${cp?"전투력 "+CombatPowerSystem.format(cp):item.damageMul.toFixed(2)+"x"}</span></strong><p>${weaponDefs[item.weapon].name} · ${item.abilityName}: ${item.abilityDesc}</p><div class="potential-mini">${item.potentials.map(x=>`<span>• ${potentialText(x)}</span>`).join("")}</div><div class="weapon-actions"><button class="equip-toggle ${eq?"is-equipped":""}" data-equip="${item.id}">${eq?"장착 해제":"장착"}</button><button data-detail="${item.id}" data-forge-detail-tab="enhance">강화</button><button data-detail="${item.id}" data-forge-detail-tab="potential">재련</button><button data-break="${item.id}">분해</button></div></div></div>`;
    ...
    ui.weaponInventory.querySelectorAll("[data-detail]").forEach(b=>b.addEventListener("click",e=>{e.stopPropagation();openDetail(b.dataset.detail,b.dataset.forgeDetailTab||"enhance")}));
    ...
    if(window.ForgeArtUI){window.ForgeArtUI.applyWeaponFilter();window.ForgeArtUI.syncWeaponInspect();}
  };
  ...
  window.CheonHaForgeV13={openDetail,normalizeWeapon,enhancementMultiplier,potentialText,MAX_ENHANCE,GRADE_ENHANCE_SCALE,failureRisk,showPotentialOdds};
})();
