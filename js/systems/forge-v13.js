...
(function installForgeV13(){
  ...
  function renderDetail(){
    ...
    const rates=enhancementOutcomeRates(item),riskBox=root.querySelector("#enhanceRisk");if(riskBox)riskBox.textContent=item.level>=MAX_ENHANCE?"최대 강화 단계":rates.guaranteed?"장인의 숨결 완성 · 다음 강화 확정 성공":`시도 기준 · 성공 ${(rates.success*100).toFixed(rates.success<.1?1:0)}% · 단계 하락 ${(rates.down*100).toFixed(1)}% · 파괴 ${(rates.destroy*100).toFixed(rates.destroy>0&&rates.destroy<.01?1:0)}% · 유지 ${(rates.stay*100).toFixed(1)}%`;
