...
(function installForgeV13(){
  ...
  // meta-menus-events.js가 구버전 함수 참조를 클릭 리스너에 저장했으므로
  // 버튼을 복제해 해당 리스너를 제거하고 v13 단조 함수만 연결한다.
  const oldForgeButton=document.getElementById("forgeBtn");
  if(oldForgeButton){
    const newForgeButton=oldForgeButton.cloneNode(true);
    oldForgeButton.replaceWith(newForgeButton);
    newForgeButton.addEventListener("click",window.forgeWeapon);
    if(window.ui)ui.forgeBtn=newForgeButton;
  }
