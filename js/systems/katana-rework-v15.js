...
const KATANA_ANIM_META={
 ...
 "jeolwol":{sheet:"jeolwol.sheet.png",sheets:["jeolwol.sheet.png","jeolwol.sheet.part2.png"],framesPerSheet:26,fw:773,fh:435,frames:52,cols:5,d:Array(52).fill(50),total:2600},
...
const KATANA_SHEETS=Object.values(KATANA_ANIM_META).flatMap(m=>m.sheets||[m.sheet]);
...
function drawAnim(name,timeSec,x,y,w,h,a=1,rot=0,loop=false,flipX=false){
 const meta=KATANA_ANIM_META[name];if(!meta)return;const fi=frameAt(meta,Math.max(0,timeSec||0)*1000,loop),sheetIndex=meta.sheets?Math.min(meta.sheets.length-1,Math.floor(fi/meta.framesPerSheet)):0,localFrame=meta.sheets?fi%meta.framesPerSheet:fi,im=img(meta.sheets?.[sheetIndex]||meta.sheet);if(!im?.complete||!im.naturalWidth)return;
 const sx=(localFrame%meta.cols)*meta.fw,sy=Math.floor(localFrame/meta.cols)*meta.fh;
