...
damageEnemy=function(e,dmg,source,opt={}){
 ...
 // 원킬이어도 은월 타격 이펙트와 은빛 피해 표시는 남는다.
 if(silver&&dealt>0){
  vfx("katanaSilverHit",hitX,hitY,.36,{r:46});
  if(!e.dead&&!scarBurst)addScar(e,1);
 }
...
function drawKatanaVisuals(){
 for(const v of visuals){
  ...
  else if(v.type==="katanaJeolwol"){f="jeolwol";w=W*1.12;h=H*1.12;rot=0}
  else if(v.type==="katanaTimeSpaceCut"){f="time_space_cut";w=W*1.08;h=w*281/500;rot=0}
...
function drawKatanaHUD(){
 ...
 for(let i=0;i<3;i++){
  const ang=-1.02+i*.52,x=cx-50-Math.cos(ang)*12,y=cy+Math.sin(ang)*42;
  ctx.save();
