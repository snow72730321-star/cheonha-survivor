...
damageEnemy=function(e,dmg,source,opt={}){
 ...
 if(eligible&&(e.moonScar||0)>=3){e.moonScar=0;vfx("katanaScarBurst",e.x,e.y,.99,{r:78});_damage(e,Math.max(1,dmg*1.8),"moonScarBurst",{katanaNoSilver:true,skipImpactVfx:true,color:"#dff7ff"});scarTrigger();}
 if(e.dead)return 0;
...
function castJeolwol(){vfx("katanaJeolwol",player.x,player.y,2.6,{screen:true,r:Math.max(W,H)});for(const e of visibleEnemies(-120))if(!e.dead)_damage(e,150+(player.arts.nameless||1)*55,"jeolwol",{katanaNoSilver:true,skipImpactVfx:true,color:"#f7fbff",shake:10});player.moonPhase=0;gainExtremeMoon();showMessage("절월 · 만월절단",1.2)}
...
const _dodge=performDodge;performDodge=function(){const before=player.metrics?.dodges||0,pb=player.metrics?.perfectDodges||0;_dodge();if(selectedWeapon!=="katana"||(player.metrics?.dodges||0)<=before)return;const perfect=(player.metrics?.perfectDodges||0)>pb;player.katanaDodgeCharge=(player.katanaDodgeCharge||0)+(perfect?2:1);if((player.arts?.zanshin||0)>0&&player.katanaDodgeCharge>=3){player.katanaDodgeCharge-=3;player.moonMeteorBuff=4.5+.35*player.arts.zanshin;player.dodgeCooldown=Math.max(.55,player.dodgeCooldown*(1-Math.min(.22,.08+player.arts.zanshin*.025)));vfx("katanaMoonBuff",player.x,player.y,1.44,{followPlayer:true,r:105});aoe(player.x,player.y,72+player.arts.zanshin*7,18+player.arts.zanshin*8,"moonMeteor",{skipVisual:true,color:"#e9f7ff",shake:4});showMessage("월하유성보",.75)}};
// 기존 왜도 자동 arts를 억제하고 새 자원 루프를 진행한다.
const _tick=tickArts;tickArts=function(dt){if(selectedWeapon!=="katana")return _tick(dt);const a=player.arts,save=[a.moonchain,a.zanshin,a.nameless];a.moonchain=0;a.zanshin=0;a.nameless=0;_tick(dt);[a.moonchain,a.zanshin,a.nameless]=save;player.moonFlowerLock=Math.max(0,(player.moonFlowerLock||0)-dt);player.moonMeteorBuff=Math.max(0,(player.moonMeteorBuff||0)-dt);player.timeSpaceBuff=Math.max(0,(player.timeSpaceBuff||0)-dt);
 for(const q of player.katanaEchoQueue||[])q.t-=dt;while(player.katanaEchoQueue?.length&&player.katanaEchoQueue[0].t<=0){const q=player.katanaEchoQueue.shift();lineHit(q.x,q.y,q.x+Math.cos(q.a)*q.len,q.y+Math.sin(q.a)*q.len,q.width,q.dmg,"balwolEcho",{skipVisual:true,shake:2,color:C.katana});vfx("katanaBalwolEcho",q.x+Math.cos(q.a)*q.len*.48,q.y+Math.sin(q.a)*q.len*.48,.72,{a:q.a,r:q.len,width:q.width});}
