...
/**
 * v14.16.5 · 천마신교 1인 레이드 · 성능/보스 HUD/엔드게임 밸런스
 *
 * 기존 생존 난이도의 스폰·혈마 승리 판정과 분리된 보스 러시 컨트롤러다.
 * 모든 공격은 raid.telegraphs에 경고 판정을 먼저 등록한 뒤에만 피해를 준다.
 */
(()=>{
  const RAID_BUILD="cheonha-solo-raid-v5-perf-bosshud";
  ...
  const RAID_MOBILE_MAP="assets/raid/map/cheonma-altar-mobile.webp";
  const STAGES=[
  ...
  let raidHudAccumulator=0,raidPartSignature="";
  ...
  function raidTick(dt){
    ...
    if(raid.phaseGrace>0){raid.phaseGrace=Math.max(0,raid.phaseGrace-dt);player.invuln=Math.max(player.invuln||0,.2);raid.previousHp=player.hp;raidHudAccumulator-=dt;if(raidHudAccumulator<=0){raidHudAccumulator=.10;updateRaidHud()}return}
    ...
    raid.previousHp=player.hp;raidHudAccumulator-=dt;if(raidHudAccumulator<=0){raidHudAccumulator=.10;updateRaidHud()}
  }
  ...
  function updateRaidHud(){
    if(!raid.active)return;const stage=STAGES[raid.stageIndex],current=raid.currentBoss;
    const final=raid.stageIndex===4,phaseName={1:"1/3 · 양대 마수",2:"2/3 · 폭주 마룡",3:"3/3 · 천마 본체"}[raid.finalPhase]||"강림 준비";
    const displayName=final?(raid.finalPhase===1?"천마의 양대 마수":current?.bossName||stage.name):stage.name;
    ...
    if(hud.gimmick){let text=stage.gimmick;if(stage.id==="ma"&&raid.parts.some(p=>p.raidId.startsWith("sword-seal-")&&!p.dead))text=`검총 붕괴까지 ${Math.max(0,raid.gimmickTimer).toFixed(1)}초`;if(stage.id==="cheondan"&&raid.shieldActive)text=`금강반진 ${Math.max(0,raid.gimmickTimer).toFixed(1)}초 · 후방 공격`;if(final&&raid.finalPhase===1)text=raid.armResonanceActive?`마기 공명 ${Math.max(0,raid.gimmickTimer).toFixed(1)}초 · 지정 마수 집중 공격`:"좌우 마수 중 어느 한쪽이든 파괴하면 마룡으로 변신";if(final&&raid.finalPhase===2)text=raid.dragonCoreActive?`마룡 심핵 ${Math.max(0,raid.gimmickTimer).toFixed(1)}초 · 집중 공격`:"마룡의 전조 범위를 회피하고 본체를 끌어내라";if(final&&raid.finalPhase===3)text=`천마 본체 10줄 · 마기침식 ${raid.magicCorruption}/3${raid.bodyEnraged?" · 천마진체":""}`;hud.gimmick.textContent=text}

    // 부위 HUD DOM은 구조가 바뀔 때만 재생성하고, 이후에는 폭(width)만 갱신한다.
    if(hud.parts){const visible=raid.parts.filter(p=>!p.raidId.startsWith("sword-seal-")||!p.dead),signature=visible.map(p=>p.raidId).join("|");if(signature!==raidPartSignature){raidPartSignature=signature;hud.parts.innerHTML=visible.map(p=>`<div class="raid-part-bar" data-raid-part="${p.raidId}"><span>${p.bossName}</span><div class="raid-part-track"><i></i></div></div>`).join("")}for(const p of visible){const row=hud.parts.querySelector(`[data-raid-part="${p.raidId}"]`),fill=row?.querySelector("i");if(fill)fill.style.width=`${Math.max(0,p.hp/p.maxHp*100)}%`;row?.classList.toggle("destroyed",!!p.dead)}}

    const wrap=ui.bossWrap,title=document.getElementById("bossTitle"),stageLabel=document.getElementById("bossStageLabel"),portrait=document.getElementById("bossFramePortrait"),subFill=document.getElementById("bossSubFill"),subText=document.getElementById("bossSubText"),pips=document.getElementById("bossPhasePips");
    wrap?.classList.add("raid-mode");if(title)title.textContent=displayName;if(stageLabel)stageLabel.textContent=final?`최종 · ${raid.finalPhase||1}/3`:`${raid.stageIndex+1}관문 · ${fmtClock(raid.stageTime)}`;
    let hpEntity=current;if(final&&raid.finalPhase===1){const target=raid.parts.filter(p=>p.raidFinalArm&&!p.dead).sort((a,b)=>a.hp/a.maxHp-b.hp/b.maxHp)[0];if(target){hpEntity=target;if(current){current.hp=target.hp;current.maxHp=target.maxHp}}}
    const portraitSrc=hpEntity?.sprite||stage.sprite;if(portrait&&portraitSrc&&portrait.src!==portraitSrc)portrait.src=portraitSrc;
    if(hpEntity&&!hpEntity.dead){let ratio=Math.max(0,hpEntity.hp/hpEntity.maxHp),label=`${Math.ceil(hpEntity.hp).toLocaleString()} / ${Math.ceil(hpEntity.maxHp).toLocaleString()}`;if(final&&raid.finalPhase===3&&hpEntity.raidId==="cheonma-body"){const lineHp=hpEntity.maxHp/FINAL_BODY_BARS,lines=Math.max(1,Math.ceil(hpEntity.hp/lineHp)),lineValue=Math.max(0,hpEntity.hp-(lines-1)*lineHp);ratio=Math.max(0,Math.min(1,lineValue/lineHp));label=`×${lines}`};ui.bossFill.style.width=`${ratio*100}%`;ui.bossText.textContent=label}
    const gimmick=!!raid.gimmickGaugeActive;wrap?.classList.toggle("gimmick-active",gimmick);if(subFill)subFill.style.width=gimmick?`${Math.max(0,Math.min(1,raid.gimmickGaugeValue/raid.gimmickGaugeMax))*100}%`:"0%";if(subText)subText.textContent=gimmick?raid.gimmickGaugeLabel:"";
    if(pips){if(pips.children.length!==7)pips.innerHTML=Array.from({length:7},()=>"<i></i>").join("");const done=raid.stageIndex<4?Math.max(0,raid.completedGates):4+Math.max(0,(raid.finalPhase||1)-1),active=raid.stageIndex<4?raid.stageIndex:4+Math.max(0,(raid.finalPhase||1)-1);[...pips.children].forEach((node,i)=>{node.className=i<done?"done":i===active?"current":""})}
  }
  ...
  async function beginRaid(){
    ...
    try{
      const raidLoadList=innerWidth<=760?[RAID_MOBILE_MAP,...RAID_ASSETS.slice(1)]:RAID_ASSETS;const [weaponResult,raidResult]=await Promise.all([GameAssets.preloadWeapon(selectedWeapon),GameAssets.preloadList(raidLoadList,()=>{},3)]);
      if(weaponResult.failed.length||raidResult.failed.length)showSystemToast("일부 레이드 에셋을 불러오지 못했습니다. 네트워크 연결을 확인해 주세요.",true);
      ensureRaidAccount();raid.active=true;raid.lastRun=false;raid.finishing=false;raid.keyConsumed=false;raid.awaitingCombat=false;raidHudAccumulator=0;raidPartSignature="";raid.stageIndex=0;raid.completedGates=0;raid.finalDamage=0;raid.finalPhase=0;raid.finalCompletedDamage=0;raid.phaseGrace=0;raid.breakTime=0;raid.bloodStacks=0;raid.healSuppression=0;raid.maFailStacks=0;raid.magicCorruption=0;raid.armResonanceActive=false;raid.dragonCoreActive=false;raid.dragonCoreTriggers={high:false,low:false};raid.bodyEnraged=false;clearGimmickGauge();raid.previousDifficulty=selectedDifficulty;selectedDifficulty="chuchul";
  ...
  drawBackground=function(){
    ...
    ctx.fillStyle="#040508";ctx.fillRect(0,0,W,H);const img=GameAssets.image(innerWidth<=760?RAID_MOBILE_MAP:RAID_ASSETS[0]);if(!img?.naturalWidth)return;
  ...
  updateHud=function(){
    ...
    ui.levelText.textContent=`경지 ${player.level}`;ui.timeText.textContent=fmtClock(raid.stageTime);ui.killText.textContent=raid.stageIndex<4?`관문 ${raid.stageIndex+1}/4`:"최종전";ui.xpFill.style.width="0%";ui.goldHud.textContent="금자 획득 없음";ui.oreHud.textContent=`열쇠 ${Math.floor(account.raidKeys||0)} · 무신석 ${Math.floor(account.divineStones||0)}`;
  };
