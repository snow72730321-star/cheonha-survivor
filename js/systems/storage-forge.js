...
function nextSynthGrade(grade){const i=gradeIndex(grade);return i>=0&&i<gradeIndex("mythic")?gradeDefs[i+1].id:null}
...
function synthesizeOre(key,max=false){
 ...
 if(!oreTypes[type]||!next){showMessage(grade==="mythic"||grade==="eternal"?"영원급 재료는 합성할 수 없다":"합성할 수 없는 광석이다",1.2);return false}
...
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>{const close=$("oreManagerClose"),modal=$("oreManagerModal");if(close)close.addEventListener("click",closeOreManager);if(modal)modal.addEventListener("click",e=>{if(e.target===modal)closeOreManager()})},{once:true});else{const close=$("oreManagerClose"),modal=$("oreManagerModal");if(close)close.addEventListener("click",closeOreManager);if(modal)modal.addEventListener("click",e=>{if(e.target===modal)closeOreManager()})}
