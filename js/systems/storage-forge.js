...
function updateForgePreview(){
 ...
 if(arr.length<3){ui.forgePreview.textContent="광석 세 개를 선택하십시오. 같은 등급은 상위 등급 확률 +22%p, 같은 종류는 +10%p와 제작 품질 보너스를 얻습니다.";return}
 ...
 const upgradeChance=base<6?((sameGrade?.22:0)+(sameType?.10:0)):0,counts={};types.forEach(t=>counts[t]=(counts[t]||0)+1);const ability=Object.keys(counts).sort((a,b)=>counts[b]-counts[a])[0];
 ui.forgePreview.innerHTML=`예상 등급 <b class="rarity-${gradeDefs[base].id}">${gradeDefs[base].name}</b>${base<6?` · 상위 등급 ${(upgradeChance*100).toFixed(0)}%`:" · 최고 등급"}<br>예상 속성 <b>${oreTypes[ability].name} · ${oreTypes[ability].ability}</b>${sameType?" · 동일 광석 품질 보너스":""}<br>필요 금자 <b>${cost.toLocaleString()}</b>`;
}
